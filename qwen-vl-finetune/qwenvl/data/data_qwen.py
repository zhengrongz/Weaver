...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        for data in dataset_list:
            ...
            if sampling_rate < 1.0:
                ...
                repeat_times = int(sampling_rate)
                remainder = sampling_rate - repeat_times
                annotations = annotations * repeat_times
                if remainder > 0:
                    extra_samples = int(len(annotations) * remainder)
                    annotations.extend(random.sample(annotations, extra_samples))
                print(f"extending to {len(annotations)} examples from dataset {data}")
            else:
    ...
    def read_video(self, video_file):
        ...
        num_frames = len(vr)
        video_length = num_frames / avg_fps
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                print(self.list_data_dict[i]['video'])
                time.sleep(1)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "video" in sources[0]:
            ...
            if isinstance(video_file, List):
                if len(video_file) > 1:
                    ...
                    # results = []
                    # results.append(self.process_video(video_file[0]))
                    # results.extend([self.read_video(file) for file in video_file[1:]])
        ...
        data_dict = preprocess_qwen_2_visual(
            chat_sources,
            self.tokenizer,
            system=sources[0].get("system", None),
