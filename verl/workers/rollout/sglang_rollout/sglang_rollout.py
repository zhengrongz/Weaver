...
from verl.agent_sg import Agent
...
class SGLangRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            with open(f'responses5/rank_{rank}.txt', 'a', encoding='utf-8') as f:
                f.write(f"--- Batch Start (Rank {rank}) ---\n")
                for i, seq in enumerate(output):
                    f.write(f"Response {i}:\n")
                    for log_prob, token_ids, _ in seq["meta_info"]["output_token_logprobs"]:   
                        f.write(self.tokenizer.decode(token_ids, skip_special_tokens=False))
                    f.write("\n---\n")
                f.write(f"--- Batch End ---\n\n")
...
class SGLangRollout_with_tool(BaseRollout):
    def __init__(
        self,
        actor_module: nn.Module | str,
        config: DictConfig,
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Extract token IDs and image data for SGLang Engine
        idx_list = [deepcopy(input_data["prompt_token_ids"]) for input_data in sglang_inputs]
        image_list = [deepcopy(input_data.get("image_data", None)) for input_data in sglang_inputs]
        
        multi_modal_inputs = non_tensor_batch.pop("multi_modal_inputs", None)
        multi_modal_inputs = np.repeat(multi_modal_inputs, n, axis=0) if multi_modal_inputs is not None else None
        ...
        non_tensor_batch["tool_use"] = np.array(tool_use, dtype=np.bool_)
