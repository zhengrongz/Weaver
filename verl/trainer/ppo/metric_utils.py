...
def compute_tool_metrics(batch: DataProto, n) -> Dict[str, Any]:
    """
    Compute metrics related to tool usage in the batch.
    """
    tool_use = batch.non_tensor_batch["tool_use"]
    # Group tools every n samples and compute statistics
    reshaped_tool_use = tool_use.reshape(-1, n)
    mean_per_group = np.mean(reshaped_tool_use, axis=1)

    # Overall statistics across all groups
    group_mean = np.mean(mean_per_group)
    group_std = np.std(mean_per_group)

    metrics = {
        "critic/tool_use/mean": group_mean,
        "critic/tool_use/std": group_std,
        "critic/acc/mean": np.mean(batch.non_tensor_batch["acc"]),
    }
    return metrics
